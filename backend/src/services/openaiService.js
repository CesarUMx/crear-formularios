...
class OpenAIService {
   ...
   */
  async generateQuestions(params, onProgress = null) {
    const {
      content,
      numberOfQuestions,
      difficulty = 'medium',
    ...
    try {
      ...
      if (questionsToRequest > MAX_QUESTIONS_PER_CALL) {
        ...
        console.log(`✂️ Generadas: ${allQuestions.length} preguntas`);
        ...
        console.log(`✂️ Generadas: ${result.questions.length} preguntas`);
        console.log(`✅ Guardando: ${finalQuestions.length} preguntas`);
   ...
   */
  async generateQuestionsBatch(content, numberOfQuestions, difficulty, topic, language, questionTypes, existingQuestions = []) {
    ...
    if (receivedCount !== expectedCount) {
      console.warn(`⚠️ OpenAI generó ${receivedCount} preguntas (esperadas: ${expectedCount})`);
    }
   ...
   */
  buildPrompt(content, numberOfQuestions, difficulty, topic, language, questionTypes, existingQuestions = []) {
...
1. ⚠️ EXACTAMENTE ${numberOfQuestions} preguntas en el array "questions"
...
⚠️ VERIFICACIÓN OBLIGATORIA ANTES DE RESPONDER ⚠️
...
`.trim();
   ...
   */
  getTypeInstructions(questionTypes) {
    const instructions = {
      ...
      data_interpretation: '- **Interpretación de Datos**: Tabla o gráfica descrita en texto para analizar',
    };
   ...
   */
  getFormatExamples(questionTypes) {
    const examples = [];
      ...
      examples.push(`
    {
      ...
      "text": "Analiza los siguientes datos",
      "dataDescription": "Descripción de tabla o gráfica con datos específicos",
      "questions": [
        {
          "text": "¿Qué tendencia se observa?",
          "options": [
      ...
      "feedback": "Explicación del análisis correcto"
    }`);
    }
   ...
   */
  async regenerateQuestion(originalQuestion, content, difficulty = 'medium', questionType = 'multiple_choice', existingQuestions = []) {
    let existingQuestionsText = '';
    if (existingQuestions.length > 0) {

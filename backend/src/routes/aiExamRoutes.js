  ...
  async (req, res) => {
    try {
      ...
      if (hasDataInterpretation) {
        ...
        if (!hasNumericData) {
          ...
          if (onlyDataInterpretation) {
            ...
            const filteredTypes = types.filter(t => t !== 'data_interpretation');
            types.length = 0;
      ...
      for (let i = 0; i < questions.length; i++) {
        ...
        let metadata = {
          questionType: questionType, // ✅ IMPORTANTE: Guardar el tipo
...
router.get('/public/:slug', async (req, res) => {
  try {
    ...
    res.json({
      ...
      accessType: exam.accessType, // Agregar tipo de acceso
    });
  } catch (error) {
...
router.post('/:id/attempts', async (req, res) => {
  try {
    ...
    const questionsWithShuffling = selectedQuestions.map((q) => {
      ...
      if (questionType === 'matching' && q.metadata?.pairs) {
        const pairs = [...q.metadata.pairs];
        const rightColumn = pairs.map(p => p.right);
      ...
      if (questionType === 'ordering' && q.metadata?.items) {
    ...
    const attempt = await prisma.aIExamAttempt.create({
      data: {
        ...
        studentId,
        sessionToken: studentSessionToken,
        selectedQuestions: questionsWithShuffling, // Guardar con metadata de aleatorización
        ipAddress,
        userAgent,
      },
    });
    ...
    // Usar la metadata ya generada en questionsWithShuffling
    const questionsForStudent = selectedQuestions.map((q, idx) => {
      ...
      return {
        ...
        helpText: q.helpText,
        points: q.points,
        metadata: savedMetadata, // Usar metadata con aleatorización ya generada
    ...
    res.json({
      attempt: {
        ...
        sessionToken: studentSessionToken, // ✅ Retornar sessionToken para guardarlo en el frontend
      },
  ...
  try {
    ...
    for (const response of responses) {
      ...
      if (response.selectedOptionId) {
        const validOption = question.options?.find(opt => opt.id === response.selectedOptionId);
        if (!validOption) {
          console.log(`❌ ERROR: selectedOptionId inválido: ${response.selectedOptionId}`);
          console.log(`   Opciones válidas:`, question.options?.map(o => ({ id: o.id, text: o.text })));
          return res.status(400).json({ 
            error: `Opción inválida para la pregunta "${question.text.substring(0, 50)}..."`,
            invalidOptionId: response.selectedOptionId,
            validOptions: question.options?.map(o => o.id)
          });
        }
      }

      // Obtener metadata de aleatorización guardada en selectedQuestions
      const savedQuestion = attempt.selectedQuestions.find((sq) => sq.id === response.questionId);
      ...
      // Calificar automáticamente si es posible usando la pregunta con metadata de aleatorización
      ...
      const responseData = {
        ...
        // Solo incluir selectedOptionId si tiene valor (no vacío)
        ...(response.selectedOptionId && response.selectedOptionId !== '' ? { selectedOptionId: response.selectedOptionId } : {}),
        // Usar userAnswer o textAnswer para respuestas de matching/ordering
        ...
        isGraded: !needsManualGrading, // Auto-graded questions are already graded
      };
    ...
    const timeSpent = Math.floor((completedAt - startedAt) / 1000);
    ...
    const completedAttempt = await prisma.aIExamAttempt.update({
      ...
      data: {
        ...
        isGraded: !hasManualQuestions, // Solo está calificado si no tiene preguntas manuales
        autoScore,
        manualScore: 0,
      },
      include: {
  ...
  try {
    ...
    const allQuestions = await prisma.aIExamQuestion.findMany({
      where: {
        ...
        id: { not: questionId }, // Excluir la pregunta actual
      },
      select: {
        text: true,
      },
    ...
    if (newQuestion.options && Array.isArray(newQuestion.options)) {
      updateData.options = {
        deleteMany: {}, // Eliminar opciones antiguas
        create: newQuestion.options.map((opt, index) => ({
          text: opt.text,
...
router.post('/:id/students', requireAuth, async (req, res) => {
  try {
    const { id } = req.params;
    const { students } = req.body; // Array de { name, email, password }
...
router.post('/:id/send-invitation', requireAuth, async (req, res) => {
  try {
    ...
    // Nota: La contraseña ya está hasheada en la BD, necesitamos la original
    // Por ahora, enviaremos un mensaje indicando que deben usar la contraseña que se les proporcionó
